/**
 * Script to start Vite dev servers for all renderers
 * This script only starts the Vite dev servers without launching Electron
 */
...
/**
 * Main function to start the Vite dev servers
 */
async function main() {
  try {
    console.log(colors.cyan('Starting Vite dev servers for all renderers...'));
    ...
    // Print instructions for manually launching Electron
    console.log(colors.cyan('\nVite dev servers are running. To launch Electron, open a new terminal and run:'));
    console.log(colors.yellow(`VITE_DEV_SERVER_URL=${urls[0]} npx electron .`));
    console.log(colors.cyan('\nEnvironment variables for all renderers:'));
    console.log(colors.yellow(`VITE_VUE_SERVER_URL=${urls[0]}`));
    console.log(colors.yellow(`VITE_SVELTE_SERVER_URL=${urls[1]}`));
    console.log(colors.yellow(`VITE_THREEJS_SERVER_URL=${urls[2]}`));
    ...
    console.error(colors.red('Error starting Vite dev servers:'), err);
    process.exit(1);
  }
}
...
// Start the Vite dev servers
main().catch((err) => {
  console.error(colors.red('Failed to start Vite dev servers:'), err);
  process.exit(1);
});
